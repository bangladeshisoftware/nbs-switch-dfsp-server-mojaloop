...
const QUOTE_URL = process.env.QUOTE_URL || 'https://quoting.mojaloop.xyz';
const ML_URL    = process.env.ML_URL    || 'https://api.mojaloop.xyz';

// ════════════════════════════════════════════════════════════
//  HELPER — DFSP currency নাও
// ════════════════════════════════════════════════════════════
...
//  Sender select এর জন্য — এই DFSP এর active merchants
// ════════════════════════════════════════════════════════════
...
// ════════════════════════════════════════════════════════════
//  POST /payment/lookup
//  Step 1 — Party lookup with DB validation + ALS
// ════════════════════════════════════════════════════════════
exports.partyLookup = async (req, res) => {
  ...
  if (!sender_id || !receiver_id_type || !receiver_id_value)
    return res.status(400).json({ error: 'sender_id, receiver_id_type, receiver_id_value required' });

  try {
    // ── 1. Sender merchant DB থেকে নাও ─────────────────
    ...
    if (sender.status !== 'ACTIVE')
      return res.status(400).json({ error: `Sender account is ${sender.status}` });
...
exports.initQuote = async (req, res) => {
  ...
  if (!sender_id || !receiver_party || !amount)
    return res.status(400).json({ error: 'sender_id, receiver_party, amount required' });
...
exports.initTransfer = async (req, res) => {
  ...
  if (!quote_id || !payer_fsp || !payee_fsp || !amount || !ilp_packet || !condition)
  ...
  try {
    ...
    const requestBody = {
      ...
      expiration:  expiration || new Date(Date.now() + 5 * 60 * 1000).toISOString(),
