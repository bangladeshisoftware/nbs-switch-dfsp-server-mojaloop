...
// ════════════════════════════════════════════════════════════
//  GET /activity-logs
//  DFSP portal: শুধু নিজের DFSP এর users এর logs দেখাবে
// ════════════════════════════════════════════════════════════
exports.getLogs = async (req, res) => {
  try {
    const { dfsp_id } = req.user; // JWT থেকে
    const {
      username,
      page  = 1,
      limit = 50,
    ...
    // এই DFSP এর সব usernames বের করো
    const [dfspUsers] = await pool.execute(
      `SELECT username FROM dfsp_users WHERE dfsp_id = ?`, [dfsp_id]
    );
...
exports.getStats = async (req, res) => {
  try {
    ...
    const [[stats]] = await pool.execute(`
      ...
      FROM activity_logs ${where}`, usernames);
