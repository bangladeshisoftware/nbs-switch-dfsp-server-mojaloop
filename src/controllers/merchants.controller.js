...
const ALS_URL = process.env.ALS_URL || 'https://als.mojaloop.xyz';

// ════════════════════════════════════════════════════════════
//  ALS Helper — Party register করো
// ════════════════════════════════════════════════════════════
...
// ALS Helper — Party delete করো (rollback এর জন্য)
...
// POST /merchants — Register new merchant + ALS party registration
exports.create = async (req, res) => {
  ...
  try {
    const {
      ...
      // ALS party fields
      id_type  = 'MSISDN',   // MSISDN | ACCOUNT_ID | BUSINESS | ALIAS
      id_value,              // ALS এ register হবে এই value দিয়ে (default: phone)
    ...
    // ALS এ কোন ID দিয়ে register করবো
    ...
    // ── Step 2: ALS এ Party register করো ─────────────────
    // DFSP এর currency দরকার
    ...
    try {
      ...
      // ALS async — 202 Accepted হলেই success
      if (alsRes.status >= 200 && alsRes.status < 300) {
        alsStatus = 'registered';
        console.log(`[MERCHANT] ALS party registered: ${id_type}/${alsIdValue} for ${dfsp_id}`);
      } else {
        alsStatus = 'failed';
      ...
      // ALS unavailable হলে warning দাও কিন্তু DB তে save করো
      alsStatus = 'failed';
      console.error(`[MERCHANT] ALS registration failed: ${alsErr.message}`);
      // ALS fail হলে block করবো কিনা?
    ...
    console.log(`[MERCHANT] Registered: ${merchantId} | ALS: ${alsStatus}`);
...
// PUT /merchants/:id/status — Approve/Suspend/Reject
...
exports.deleteMerchant = async (req, res) => {
  ...
  try {
    ...
    // ── Step 2: ALS থেকে party delete করো ───────────────
    ...
    if (merchant.als_status === 'registered' && idValue) {
      try {
        ...
        // ALS 404 = already gone — OK
        if (alsErr.response?.status === 404) {
          alsStatus = 'not_found';
          console.warn(`[MERCHANT] ALS party already gone: ${idType}/${idValue}`);
        } else {
          alsStatus = 'failed';
          console.error(`[MERCHANT] ALS delete failed: ${alsErr.message}`);
          // ALS fail হলেও DB থেকে delete করবো
        }
      }
    }
    ...
    console.log(`[MERCHANT] Deleted: ${merchant.merchant_id} | ALS: ${alsStatus}`);
    ...
    console.error(`[MERCHANT] Delete error: ${err.message}`);
    res.status(500).json({ error: err.message });
  }
};
